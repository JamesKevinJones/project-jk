...
- **PUSAB and Luminari are named in the CSS stack but must never be committed.** PUSAB is freeware that permits commercial use but grants no redistribution right; Luminari is proprietary to Apple. To render either locally, put the file in `hud/fonts-local/` via `scripts/add-local-font.py` -- that folder is gitignored and served over localhost, which also works when the browser cannot see a system-installed font because it enumerated fonts at process start. The repo vendors Lilita One and Grenze (both SIL OFL) as fallbacks. If you are ever tempted to "fix" the missing fonts by committing them, don't.
...
- **Full-bleed chrome spans `grid-column: 1 / -1`; it is never absolutely positioned inside a column.** The status bar sits in the `.story` grid across both columns, which is what makes its rules run edge to edge. The earlier version put that text inside the half-width sticky stage, so once the stage scrolled partly out of view its borders terminated mid-screen and read as a broken box. The stage's `top` offset and height both derive from `--status-h`; change the bar's height only through that token or the two will overlap.
