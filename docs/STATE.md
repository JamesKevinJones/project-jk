...
The console (`python scripts/hud.py`, or the `J.K. HUD.bat` desktop shortcut) serves a page at `http://127.0.0.1:7842` in two halves: a five-beat GSAP ScrollTrigger story over the vault's real link graph, then the live console. The visual language is StarMatch's neo-brutalism. GSAP is vendored in `hud/vendor/`; Lilita One, Grenze and IBM Plex Mono are downloaded into `hud/fonts/` by `scripts/fetch-fonts.py` as the open-licensed fallbacks for PUSAB and Luminari. The page makes no external requests.
